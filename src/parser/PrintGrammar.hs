...
instance Print Expr where
  prt i e = case e of
    ...
    Case expr alts -> prPrec i 0 (concatD [doc (showString "case"), prt 2 expr, doc (showString "of"), doc (showString "{"), prt 0 alts, doc (showString "}")])
...
instance Print Ty where
  prt i e = case e of
    ...
    TList ty -> prPrec i 1 (concatD [doc (showString "List"), prt 0 ty])
...
instance Print Alt where
  prt i e = case e of
    AltCase toppattern expr -> prPrec i 0 (concatD [prt 0 toppattern, doc (showString "->"), prt 0 expr])
  prtList _ [x] = (concatD [prt 0 x])
  prtList _ (x:xs) = (concatD [prt 0 x, doc (showString ";"), prt 0 xs])
instance Print TopPattern where
  prt i e = case e of
    TopPatternAt id pattern -> prPrec i 0 (concatD [prt 0 id, doc (showString "@"), prt 0 pattern])
    TopPatternNo pattern -> prPrec i 0 (concatD [prt 0 pattern])

instance Print Pattern where
  prt i e = case e of
    PatData id patconstrargs -> prPrec i 0 (concatD [prt 0 id, prt 0 patconstrargs])
    PatBind bind -> prPrec i 0 (concatD [prt 0 bind])
    PatLit lit -> prPrec i 0 (concatD [prt 0 lit])
    PatIdent id -> prPrec i 0 (concatD [prt 0 id])
    PatDefault -> prPrec i 0 (concatD [doc (showString "_")])
    PatHeadIdent id pattern -> prPrec i 0 (concatD [prt 0 id, doc (showString ":"), prt 0 pattern])
    PatHeadLit lit pattern -> prPrec i 0 (concatD [prt 0 lit, doc (showString ":"), prt 0 pattern])

instance Print PatConstrArg where
  prt i e = case e of
    PatConstrArgDef id -> prPrec i 0 (concatD [prt 0 id])
  prtList _ [x] = (concatD [prt 0 x])
  prtList _ (x:xs) = (concatD [prt 0 x, prt 0 xs])
